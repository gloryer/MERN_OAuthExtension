...
```
 Multiple actions are allowed in one rule.  The ` actions `contains a list of strings representing different actions. Each action has an object which parameterizes all the authorization conditions.  The example below shows two allowed actions and the associated consitions. 
